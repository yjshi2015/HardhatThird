...
/**
 * @title 恒定乘积做市商，非常经典的DEX合约
 * @author 
 * @notice 
 * 1.恒定乘积做市商：k=x*y,并不是说k永远不变，而是在发生swap时，本次的k值不变。当交易完成后，k值
 *   就会成为新值k1，并且k1 > k。这种设计是为了避免恒定总额做市商（k=x+y)的缺点：当x足够大时，会
 *   耗尽y的流动性；
 * 2.恒定乘积做市商同样也存在缺点：三明治攻击。假如普通用户A用amount数量的token0购买token1，此时
 *   hacker B只要实现如下顺序就可以完成攻击：
 *     ①hacker B发起同样的交易，通过提高gas保证自己的交易被优先打包（并且保证是紧贴A的交易，中间
 *      如果有其他交易会影响到交易池）；
 *     ②普通用户A发起交易（此时交易池已失衡，token1数量因第1步而变小，变得稀缺，故A换得的token1
 *      数量肯定小于hacker B)；
 *     ③hacker B使用第1步得到的token1，全部换回amount'数量的token0，此时amount' > amount，（此
 *      时token1稀缺，所以能换回更多地token0），完成攻击； 
 */
